...
extern "C" JNIEXPORT jstring JNICALL
Java_ia_off_NativeMemoryGateway_nativeExportStructural(JNIEnv* env, jobject, jlong handle, jstring request_json) {
    auto* runtime = from_handle(handle);
    if (!runtime) {
        throw_illegal_state(env, "Memoria.ia runtime is closed");
        return nullptr;
    ...
    try {
        ...
        memoria_mobile_buffer out{nullptr, 0};
        const auto status = memoria_mobile_export_structural_text_json(runtime, in, &out);
        const std::string response = take_response(out);
        if (status != MEMORIA_MOBILE_OK) {
            throw_illegal_state(env, "Falha ao exportar trilha estrutural da Memoria.ia V2");
            return nullptr;
        }
        return env->NewStringUTF(response.c_str());
    } catch (const std::exception& e) {
