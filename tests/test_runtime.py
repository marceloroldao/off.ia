...
def test_memory_context_flows_to_language_adapter():
    memoria = FakeMemoria(); language = FakeLanguage()
...
def test_baseline_bypasses_memoria_and_forwards_full_context():
    ...
    result = OfflineRuntime(memoria, language).chat("What voltage is the main supply?", mode="baseline", baseline_context=history)
